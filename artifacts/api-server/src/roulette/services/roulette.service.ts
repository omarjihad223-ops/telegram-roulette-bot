    ...
    await session.withTransaction(async () => {
      ...
      if (mode === 'daily') {
       ...
       if (noWinWeight > 0) drawItems.push({ ref: null, weight: noWinWeight });
        const forceDailyNoPrize = mode === 'daily' && user.dailyPrizeBlockedNextSpin;
        const candidate = forceDailyNoPrize ? null : drawItems.length > 0 ? weightedPick(drawItems).ref : null;
      ...
      if (!candidate) {
       ...
       const reserved = await tryReserveStock(candidate, mode, session);
      if (!reserved) {
      ...
      const task = await createClaimTaskForPrize(userPrize, {
        requiredCount: settings.wheelClaimReferralsRequired ?? settings.claimReferralsRequired,
        session,
        now,
      });
