...
export async function redeemGiftLink(token: string, telegramId: number) {
  ...
  const claimed = await GiftLink.findOneAndUpdate(
    ...
    { $set: { status: 'redeemed', redeemedByTelegramId: telegramId, redeemedAt: now } },
    { new: true }
  );
  if (!claimed) {
  ...
  try {
    ...
    await GiftLink.updateOne({ _id: claimed._id, status: 'redeemed' }, { $set: { status: 'unused', redeemedByTelegramId: null, redeemedAt: null } });
    throw err;
  }
}